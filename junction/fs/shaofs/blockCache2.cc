...
void init_block_cache2(size_t capacity, size_t shard_num)
{
...
bool bc_read(BlockID id, void* buffer)
{
    ...
    auto handle = cache.get(id);    // 获取这个 CacheEntry 的 handle，这一步是 blocking 的（如果 Cache Miss，内部会自动调用 backend->read() 加载数据）
    if (!handle) 
    {
...
void bc_write(BlockID id, const void* buffer)
{
    auto& cache = get_block_cache();
    cache.put(id, *(BlockData*)buffer);
}
void bc_flush_all()
{
    auto& cache = get_block_cache();
    log_info("Flushing all dirty blocks to backend...");
    cache.flush();
}

BlockHandle bc_get_handle(BlockID id)
{
    auto& cache = get_block_cache();
    return cache.get(id);
}
