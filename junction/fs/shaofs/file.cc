...
{   
    ...
    while (bytes_read < len)
    {
...

        {
            ...
            phys_blk = inode_bmap_locked(const_cast<MInode*>(&(*read_acc)), logical_blk, false, nullptr);  // 直接调用 bmap，内部可能会需要读取间接块，发生 IO 阻塞，当前线程会带锁休眠（不影响其他 reader）
        } // 读锁释放
...
ssize_t file_write_direct(int inum, const char* buf, off_t offset, size_t len)
{
    ...
    while (bytes_written < len)
    {
        ...
        // TODO: 将 Cache 中可能存在的旧页面强制失效  bc_invalidate_block(phys_blk);
