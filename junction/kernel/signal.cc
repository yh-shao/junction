...
extern "C" void synchronous_signal_handler(int signo, siginfo_t *info,
  ...
  assert_on_runtime_stack();
  assert_stack_is_aligned();
  write_uint("sync signal signo = ", signo);
  if (info) write_uint("sync signal addr  = ", (uint64_t)info->si_addr);
  ...
  auto uc = k_sigframe::FromUcontext(reinterpret_cast<k_ucontext *>(context));
  write_uint("sync signal RIP   = ", uc->uc.uc_mcontext.rip);
  write_uint("sync signal RSP   = ", uc->uc.uc_mcontext.rsp);
